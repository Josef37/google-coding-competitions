    ...
    for move_number in range(K+1):
      ...
      if move[0] == "E":
        ...
        if lo <= estimate and estimate <= hi:
          print(f"Case #{case_number}: Correct -- got {estimate}; exact answer is {correct_total_edges}. Off by {round(abs(estimate / correct_total_edges - 1) * 100, 2)} %", file=sys.stderr)
          correct += 1
        else:
